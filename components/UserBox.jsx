...
const UserBox = ({ data, type, loggedInUserData }) => {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onError: (err, variables, context) => {
      ...
      console.error("Lỗi. Vui lòng thử lại sau!", err);
      queryClient.setQueryData(
        ["user", currentUser?.id, "followInfo"],
    ...
    <Box
      ...
      onClick={() =>
        window.location.assign(
          `/profile/${data?.[type]?.id}?person=${data?.[type]?.first_name} ${data?.[type]?.last_name}`
        )
      }
    >
            ...
            >
              <Flex gap={10} align="center">
                ...
                <Typography.Text strong style={{ color: "#3db66a" }}>
                  {isPending ? "Đang tải..." : "Đang theo dõi"}
                </Typography.Text>
              </Flex>
            </Button>
          )}
