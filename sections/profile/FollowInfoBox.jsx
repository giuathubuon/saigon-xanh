...
const FollowInfoBox = ({ id }) => {
  ...
  if (isError) return <Alert message="Lỗi tải dữ liệu!" type="error" />;

  return (
    <Box className={css.container}>
      <Space direction="vertical" align="center">
        <Typography className={"typoH5"}>{data?.followers?.length}</Typography>
        <Typography className={"typoSubtitle2"}>Người theo dõi</Typography>
      </Space>

      <Space direction="vertical" align="center">
        <Typography className={"typoH5"}>{data?.following?.length}</Typography>
        <Typography className={"typoSubtitle2"}>Đang theo dõi</Typography>
      </Space>
    </Box>
  );
};
